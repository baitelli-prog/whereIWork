...
const MAX_POINTS = 5000;   // accident breaker on geocode calls per draw (~$25 max)
const MAX_TRACES = 2000;  // hard cap on Tracerfy lookups per group ($0.20 each = $400 max)
const GRID_SPACING_M = 40; // meters between sample points; finer grid = more addresses
...
export default function RevealMap({ onCreateGroup, boardLeads = [], focusLead, onOpenLead }) {
  ...
  const sampleAddresses = useCallback(async (path) => {
    ...
    const inside = [];
    // grid step in degrees from a meter spacing (lat ~111km/deg; lng scales by cos(lat))
    const midLat = (minLat + maxLat) / 2;
    const stepLat = GRID_SPACING_M / 111000;
    const stepLng = GRID_SPACING_M / (111000 * Math.cos(midLat * Math.PI / 180));
    for (let lat = minLat; lat <= maxLat; lat += stepLat) {
      for (let lng = minLng; lng <= maxLng; lng += stepLng) {
        if (m.geometry.poly.containsLocation(new m.LatLng(lat, lng), poly)) inside.push({ lat, lng });
        if (inside.length >= MAX_POINTS) break;
      }
      if (inside.length >= MAX_POINTS) break;
    }
    poly.setMap(null);
    ...
    setNote(inside.length >= MAX_POINTS ? `Large area — sampling the first ${MAX_POINTS} points.` : "");
    ...
    for (let k = 0; k < list.length; k++) {
      ...
      try {
        ...
        if (!error && data?.found) out.push({
          ...
          lat: data.lat ?? c.lat, lng: data.lng ?? c.lng, roofCat: data.roof_category, ownerOccupied: data.owner_occupied, hasPool: data.has_pool,
        });
      } catch {}
  ...
  const exportCSV = () => {
    const rows = [["first_name", "last_name", "full_name", "address", "city", "state", "zip", "phone", "email", "pool", "neighborhood", "offer"]];
    includedRevealed.forEach((r) => {
      ...
      rows.push([first, last, r.name || "", street || "", city || "", st || "", zip || "", safePhone, r.email || "", r.hasPool ? "Yes" : "No", neighborhood || "", offerText()]);
    });
  ...
  const create = async () => {
    ...
    try {
      await onCreateGroup({
        ...
        prospects: includedRevealed.map((r) => ({ name: r.name, addr: r.addr, phone: r.phone, email: r.email, value: r.value, equity: r.equity, beds: r.beds, sqft: r.sqft, year: r.year, lat: r.lat, lng: r.lng, dnc: r.dnc, hasPool: r.hasPool })),
      });
      setCreated(true);
            ...
            {revealed.map((r) => { const off = excluded[r.id]; return (
              <div key={r.id} style={{ border: `1px solid ${off ? C.line : C.accent}`, opacity: off ? 0.5 : 1, borderRadius: 12, padding: 12, marginBottom: 10 }}>
                ...
                <div style={{ fontSize: 13, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                  ...
                  {r.hasPool && <span style={{ background: "#e0f2fe", color: "#0369a1", fontSize: 11, padding: "1px 6px", borderRadius: 6 }}>🏊 Pool</span>}
